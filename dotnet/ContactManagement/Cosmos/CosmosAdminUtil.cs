...
{
    /**
     * This class is used for Cosmos DB administrative tasks, such as creating databases and containers,
     * listing them, and updating container throughput.
     *
    ...
    public class CosmosAdminUtil : CosmosBaseUtil
    {
        ...
        public async Task<List<string>> ListContainers(string dbname)
        {
            ...
            Console.WriteLine("ListContainers dbname: " + dbname);
            try
            {
                Database db = await GetDatabase(dbname);
                    ...
                    if (loopCount < 10000)
                    {
                        ...
                        else
                        {
                            foreach (GenericDocument gd in genericDocs)
                            {
                                ...
                                Console.WriteLine(
                                    $"{loopCount} {deleteOperationCount} {ir.StatusCode} {ir.RequestCharge} {gd.ToJson()}");
                            }
