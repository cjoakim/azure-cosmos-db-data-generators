...
{
    /**
     * This class is used to execute various Cosmos DB queries and return QueryResponse objects.
     * The QueryResponse object contains the actual response documents, as well as metadata
     * such as RU costs of the query.
     *
    ...
    public class CosmosQueryUtil : CosmosBaseUtil
    {
        ...
        public async Task<QueryResponse> ExecuteParameterizedQuery(QueryDefinition queryDef, bool verbose = false)
        {
            QueryResponse respObj = new QueryResponse();
            try
            {
                if (verbose)
                {
                    Console.WriteLine($"ExecuteParameterizedQuery - sql: {queryDef.QueryText}");
                }
                QueryRequestOptions qOpts = new QueryRequestOptions();
                FeedIterator<dynamic> qIter =
                    this.currentContainer.GetItemQueryIterator<dynamic>(
                        queryDef, requestOptions: qOpts);
            ...
            {
                Console.WriteLine($"ExecuteParameterizedQuery -> Exception {e}");
            }
